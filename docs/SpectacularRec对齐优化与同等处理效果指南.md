...
> 说明：本应用当前已实现“**双产物**”以兼顾 Spectacular 兼容性与现场记录需求：  
> - `data.mov`：永远保持 **纯视频（无音轨）**，与 Spectacular 样例合同一致；  
> - `data_with_audio.mov`：若开启音频并成功录到，会额外导出一份 **带音轨** 的视频用于回放/排查。  
> 因此给 Spectacular CLI 的目录可直接使用 `data.mov`，无需再做去音轨处理（旧数据/外部视频仍可参考 §7.4 的 ffmpeg 工作流）。
...
- **主路视频**：`data.mov`（H.264；**无音轨**，Spectacular-compatible）
- **带音频版本（可选）**：`data_with_audio.mov`（H.264 + 音频轨；仅当开启音频且导出成功时生成）
...
- `data.mov`：始终保持 **纯视频（无音轨）**，作为 Spectacular CLI / 下游工具链输入  
- `data_with_audio.mov`：同一段视频 + 音频轨，留作现场记录、回放与额外处理

本仓库当前 iOS 实现（`ios/Runner/SlamRecordingSession.swift`）已按上述方式工作：

- 录制时：`data.mov` 永远只写 video stream；音频单独采集。
- 停止录制时：若音频确实录到，会自动导出 `data_with_audio.mov`（优先 passthrough；必要时回退重编码）。
- 过程中可能会暂存 `audio.m4a`；导出成功会清理，导出失败会保留用于排查/手工合成。

因此给 Spectacular CLI 的目录可以直接使用 `data.mov`，无需再额外“去音轨”步骤。

**旧数据/外部视频兼容（仍带音轨的 `data.mov`）**：如果你手头已有“带音轨的 `data.mov`”，仍可用 ffmpeg 生成无音轨版本（仅视频流）：
