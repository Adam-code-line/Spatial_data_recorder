...
| 产物/字段                                                                        | 说明                                                                                                                                                                                                                                                                                                |
| -------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`data.mov` 视频内容**                                                          | 广角主路：来自 `AVCaptureVideoDataOutput` 的实时帧，经 `AVAssetWriter` 编码为 H.264。为对齐 Spectacular Rec / Spectacular CLI 输入合同，建议 **`data.mov` 保持无音轨**；若需保留音频用于额外处理，建议以 sidecar 文件或额外导出版本保留（例如 `audio.m4a` / `data_cli.mov`）。 |
...
| **`frames2/*.png`（若存在）**                                                    | **深度模式**下为逐帧 PNG 灰度深度可视化图；**MultiCam 回退**下为第二路 RGB 逐帧图（不再伪装成灰度深度），用于与 `data2.mov` 保持逐帧对应。                                                                                                                                                          |
