...
| ------------------ | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| 视频               | `data.mov`（+ 样例可有 `data2`）                                | ✅ **仅 `data.mov`**（不再落盘 `data2.mov`）                                                |
| JSONL              | 含陀螺仪、加速度计、**磁力计**、**双相机帧**（含 depth 元数据） | ✅ **加速度计 + 陀螺仪 + 磁力计** + **`frames`（单路或双路，双路时第二路由 `frames[1]` 描述）** |
...
- `data.mov`（H.264 视频；**为 Spectacular CLI 对齐建议保持无音轨**）
...
补充：如果你需要“保留音频做额外处理”，推荐把音频作为**额外文件或额外导出版本**保留（例如 `audio.m4a` / `data_cli.mov`），以便把“给 Spectacular CLI 的目录”维持为样例合同。
...
- [x] **多相机 / 深度第二路**：优先 **同步器 + 深度输出**（LiDAR 等）→ 导出 `frames2/*.png` + JSONL **gray/depthScale**；否则 **MultiCam 广角+超广角** 双 RGB（第二路仍通过 `frames2/*.png` + `frames[1]` 表达，不再写 `data2.mov`）；见 [`Spectacular样例对齐差距说明.md`](Spectacular样例对齐差距说明.md)。
...
| 包含                                                                  | 不包含（本阶段不做）                                            |
| --------------------------------------------------------------------- | --------------------------------------------------------------- |
| `data.mov` + `frames2/*.png`（双路模式；`data.mov` 建议无音轨；不再写 `data2.mov`） | 原始 Float 深度侧车文件、非 H.264 深度裸流                      |
...
说明：当前上传链路对 `frames2` 有必需校验（目录存在且至少 1 个文件）。
...
- **可选采集麦克风（用于额外处理）**：如果你需要保留音频，请将其视为“附加信息”，不要破坏 Spectacular 样例合同。
  - 推荐：`data.mov` 仍保持仅视频流；音频写入 `audio.m4a`（sidecar）或导出 `data_cli.mov`（无音轨）供 Spectacular CLI 使用。
  - 如果你选择把音频直接写进 `data.mov`：请确保在跑 Spectacular CLI 前有一个“去音轨版本”的输入目录，否则可能遇到脚本/工具假设不一致。
