...
|------|---------------------------------------------|-------------------------|
| 视频 | `data.mov` | ✅ 单路主相机 → `data.mov`（无麦克风音轨） |
| JSONL | 含陀螺仪、加速度计、**磁力计**、**双相机帧**（含 depth 元数据） | ✅ **加速度计 + 陀螺仪** + 单目 `frames`；磁力计**可选延后** |
| `calibration.json` | 双相机 `pinhole` + `imuToCamera` | ✅ **单相机** + 占位/静态内参 + 单位阵或简单 `imuToCamera` |
...
- 磁力计行（若算法需要）。
- 多相机 / 深度：仅当产品有明确需求再开；样例为双机位，与单目 MVP 独立成阶段。
...
| 包含 | 不包含（本阶段不做） |
|------|----------------------|
| 单目 `data.mov`（无麦克风） | 双相机、深度流 |
| `data.jsonl`：accel + gyro + 单目 frames | 磁力计（可列在 P2） |
| `calibration.json`：单相机占位 | 精密现场标定流程 |
