...
补充：如果你需要“保留音频做额外处理”，推荐把音频作为**额外导出版本**保留（例如 `data_with_audio.mov`），以便把“给 Spectacular CLI 的目录”维持为样例合同（`data.mov` 仍为纯视频）。
...
- **可选采集麦克风（用于额外处理）**：如果你需要保留音频，请将其视为“附加信息”，不要破坏 Spectacular 样例合同。
  - 推荐：`data.mov` 仍保持仅视频流；额外导出 `data_with_audio.mov`（带音轨）用于回放/额外处理。
